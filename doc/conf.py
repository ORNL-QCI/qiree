...
copyright = '{:%Y}, UT–Battelle/ORNL and QIR-EE team'.format(
...
version = qiree_config['version']
release = qiree_config['release']
html_title = f"QIR-EE {version} documentation"

# Set nobreathe, sphinxbib, etc for use in 'only' directives.
for (opt, val) in qiree_config['options'].items():
...
if qiree_config['options']['sphinxbib']:
    import pybtex
...
# html_logo = "_static/qiree-square.svg"

if html_theme == 'alabaster':
    html_theme_options = {
        'github_button': True,
        'github_user': 'qiree-project',
...
# and documents repo:
 'maketitle': r"\includepdf[pages=-]{ornltm-header-qiree.pdf}",
...
latex_documents = [
    ('index', 'QIR-EE.tex', 'QIR-EE User Manual',
     author, 'howto'),
]
